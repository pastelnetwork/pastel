...
protected:
    ...
    bool Read(const K& key, T& value)
    {
        ...
        ssKey.reserve(1000);
        ssKey << key;
        Dbt datKey(&ssKey[0], static_cast<u_int32_t>(ssKey.size()));
    ...
    bool Write(const K& key, const T& value, bool fOverwrite = true)
    {
        if (!pdb)
            return false;
        if (fReadOnly)
            assert(!"Write called on database in read-only mode"); //-V547
        ...
        ssValue << value;
        Dbt datValue(&ssValue[0], static_cast<u_int32_t>(ssValue.size()));
    ...
    bool Erase(const K& key)
    {
        if (!pdb)
            return false;
        if (fReadOnly)
            assert(!"Erase called on database in read-only mode"); //-V547
