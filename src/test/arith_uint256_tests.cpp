// Copyright (c) 2011-2013 The Pastel Core developers
...
BOOST_AUTO_TEST_CASE( comparison ) // <= >= < >
{
    ...
    for (unsigned int i = 0; i < 256; ++i) {
        TmpL= OneL<< i;
        BOOST_CHECK( TmpL >= ZeroL && TmpL > ZeroL && ZeroL < TmpL && ZeroL <= TmpL); //-V501
        BOOST_CHECK( TmpL >= 0 && TmpL > 0 && 0 < TmpL && 0 <= TmpL); //-V501
        TmpL |= R1L;
