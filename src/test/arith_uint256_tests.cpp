...
std::string ArrToStr(const unsigned char A[], unsigned int width)
{
...
TEST(arith_uint256_tests, basic)
{
    ...
    EXPECT_TRUE(OneL.ToString() != ArrToStr(ZeroArray,32));
    ...
    EXPECT_TRUE(arith_uint256("0x"+R1L.ToString()) == R1L);
    EXPECT_TRUE(arith_uint256("0x"+R2L.ToString()) == R2L);
    EXPECT_TRUE(arith_uint256("0x"+ZeroL.ToString()) == ZeroL);
    EXPECT_TRUE(arith_uint256("0x"+OneL.ToString()) == OneL);
    EXPECT_TRUE(arith_uint256("0x"+MaxL.ToString()) == MaxL);
    EXPECT_TRUE(arith_uint256(R1L.ToString()) == R1L);
    EXPECT_TRUE(arith_uint256("   0x"+R1L.ToString()+"   ") == R1L);
    EXPECT_TRUE(arith_uint256("") == ZeroL);
    EXPECT_TRUE(R1L == arith_uint256(R1ArrayHex));
    ...
    EXPECT_TRUE( (R1L & arith_uint256("0xffffffffffffffff")) == arith_uint256(R1LLow64));
    EXPECT_TRUE(ZeroL == arith_uint256(0));
    EXPECT_TRUE(OneL == arith_uint256(1));
    EXPECT_TRUE((arith_uint256("0xffffffffffffffff") = arith_uint256(0xffffffffffffffffULL)) == arith_uint256(0xffffffffffffffffULL) );
...
TEST(arith_uint256_tests, plusMinus)
{
    ...
    EXPECT_TRUE(R1L+R2L == arith_uint256(R1LplusR2L));
    TmpL += R1L;
...
TEST(arith_uint256_tests, methods) // GetHex SetHex size() GetLow64 GetSerializeSize, Serialize, Unserialize
{
    ...
    TmpL.SetHex(ZeroL.ToString()); EXPECT_TRUE(TmpL == 0);
    TmpL.SetHex(HalfL.ToString()); EXPECT_TRUE(TmpL == HalfL);
