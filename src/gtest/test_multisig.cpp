...
TEST_P(PTest_Multisig, multisig_verify)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        s = sign_multisig(a_and_b, keys, txTo[0], 0, consensusBranchId);
        EXPECT_TRUE(!VerifyScript(s, a_and_b, flags, MutableTransactionSignatureChecker(&txTo[0], 0, amount), consensusBranchId, &err));
        EXPECT_EQ(err, SCRIPT_ERR_INVALID_STACK_OPERATION);

        keys.assign(1,key[1]);
        keys.push_back(key[i]);
        s = sign_multisig(a_and_b, keys, txTo[0], 0, consensusBranchId);
        EXPECT_TRUE(!VerifyScript(s, a_and_b, flags, MutableTransactionSignatureChecker(&txTo[0], 0, amount), consensusBranchId, &err));
        EXPECT_EQ(err, SCRIPT_ERR_EVAL_FALSE);
    }
    ...
    s << OP_0 << OP_1;
    EXPECT_TRUE(!VerifyScript(s, a_or_b, flags, MutableTransactionSignatureChecker(&txTo[1], 0, amount), consensusBranchId, &err));
    EXPECT_EQ(err , SCRIPT_ERR_SIG_DER);
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
        {
            ...
            if (i < j && i < 3 && j < 3)
            {
                EXPECT_TRUE(VerifyScript(s, escrow, flags, MutableTransactionSignatureChecker(&txTo[2], 0, amount), consensusBranchId, &err));
                EXPECT_EQ(err , SCRIPT_ERR_OK);
            }
            else
            {
