...
class PTest_bloom: public TestWithParam<tuple<uint32_t, double, uint32_t, uint8_t, string>>
{};

TEST_P(PTest_bloom, bloom_create_insert_serialize) 
{
    const auto &elements = get<0>(GetParam());
    const auto &fPRate = get<1>(GetParam());
    const auto &tweak = get<2>(GetParam());
    const auto &flagsIn = get<3>(GetParam());
    const auto &str = get<4>(GetParam());

    CBloomFilter filter(elements, fPRate, tweak, flagsIn);
    ...
    v_uint8 vch = ParseHex(str.c_str());
    ...
    EXPECT_TRUE( filter.contains(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8"))) << "BloomFilter doesn't contain just-inserted object!";
    filter.clear();
    EXPECT_TRUE( !filter.contains(ParseHex("99108ad8ed9bb6274d3980bab5a85c048f0950c8"))) << "BloomFilter should be empty!";
}

INSTANTIATE_TEST_SUITE_P(bloom_create_insert_serialize, PTest_bloom, Values(
    make_tuple(3, 0.01, 0, BLOOM_UPDATE_ALL, string("03614e9b050000000000000001")),
...
TEST_F(test_bloom, bloom_create_insert_key)
{
    ...
    filter.insert(v_uint8(hash.begin(), hash.end()));

    CDataStream stream(SER_NETWORK, PROTOCOL_VERSION);
    stream << filter;

    v_uint8 vch = ParseHex("038fc16b080000000000000001");
    vector<char> expected(vch.size());
...
TEST_F(test_bloom, bloom_match)
{
    ...
    EXPECT_TRUE(filter.IsRelevantAndUpdate(tx)) << "Simple Bloom filter didn't match manually serialized tx hash";
    ...
    {
        v_uint8 data(32 + sizeof(unsigned int));
        memcpy(&data[0], prevOutPoint.hash.begin(), 32);
        memcpy(&data[32], &prevOutPoint.n, sizeof(unsigned int));
        filter.insert(data);
    }
    EXPECT_TRUE(filter.IsRelevantAndUpdate(tx)) << "Simple Bloom filter didn't match manually serialized COutPoint";

    filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    filter.insert(uint256S("00000009e784f32f62ef849763d4f45b98e07ba658647343b915ff832b110436"));
    EXPECT_TRUE(!filter.IsRelevantAndUpdate(tx)) << "Simple Bloom filter matched random tx hash";

    filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    filter.insert(ParseHex("0000006d2965547608b9e15d9032a7b9d64fa431"));
    EXPECT_TRUE(!filter.IsRelevantAndUpdate(tx)) << "Simple Bloom filter matched random address";
...
TEST_F(test_bloom, merkle_block_1)
{
    ...
    EXPECT_EQ(merkleBlock.header.GetHashCurrent(), block.GetHashCurrent());

    EXPECT_EQ(merkleBlock.vMatchedTxn.size() , 1U);
    ...
    EXPECT_EQ(merkleBlock.vMatchedTxn.size() , 2U);

    EXPECT_EQ(merkleBlock.vMatchedTxn[1] , pair);

    EXPECT_EQ(merkleBlock.vMatchedTxn[0].second , uint256S("0xdd1fd2a6fc16404faf339881a90adbde7f4f728691ac62e8f168809cdfae1053"));
    EXPECT_EQ(merkleBlock.vMatchedTxn[0].first , 7U);
...
TEST_F(test_bloom, merkle_block_2)
{
    ...
    EXPECT_EQ(merkleBlock.vMatchedTxn.size() , 4U);

    EXPECT_EQ(pair , merkleBlock.vMatchedTxn[0]);

    EXPECT_EQ(merkleBlock.vMatchedTxn[1].second , uint256S("0x28204cad1d7fc1d199e8ef4fa22f182de6258a3eaafe1bbe56ebdcacd3069a5f"));
    EXPECT_EQ(merkleBlock.vMatchedTxn[1].first , 1U);

    EXPECT_EQ(merkleBlock.vMatchedTxn[2].second , uint256S("0x6b0f8a73a56c04b519f1883e8aafda643ba61a30bd1439969df21bea5f4e27e2"));
    EXPECT_EQ(merkleBlock.vMatchedTxn[2].first , 2U);

    EXPECT_EQ(merkleBlock.vMatchedTxn[3].second , uint256S("0x3c1d7e82342158e4109df2e0b6348b6e84e403d8b4046d7007663ace63cddb23"));
    EXPECT_EQ(merkleBlock.vMatchedTxn[3].first , 3U);
...
TEST_F(test_bloom, merkle_block_2_with_update_none)
{
    ...
    EXPECT_EQ(merkleBlock.vMatchedTxn.size() , 3U);
    ...
    EXPECT_EQ(merkleBlock.vMatchedTxn[2].second , uint256S("0x3c1d7e82342158e4109df2e0b6348b6e84e403d8b4046d7007663ace63cddb23"));
    EXPECT_EQ(merkleBlock.vMatchedTxn[2].first , 3U);
...
TEST_F(test_bloom, merkle_block_3_and_serialize)
{
    ...
    v_uint8 vch = ParseHex("");
    vector<char> expected(vch.size());

    for (unsigned int i = 0; i < vch.size(); i++)
        expected[i] = (char)vch[i];

    auto merkleIt = merkleStream.begin();
    auto expIt = expected.begin();
    while(expIt != expected.end())
    {
        EXPECT_EQ(*merkleIt, *expIt);
        merkleIt++;
        expIt++;
    }
}
...
TEST_F(test_bloom, merkle_block_4)
{
    ...
    EXPECT_EQ(merkleBlock.vMatchedTxn[0].second , uint256S("0x02981fa052f0481dbc5868f4fc2166035a10f27a03cfd2de67326471df5bc041"));
    EXPECT_EQ(merkleBlock.vMatchedTxn[0].first , 3U);
...
TEST_F(test_bloom, rolling_bloom)
{
    ...
    for (int i = 299; i < DATASIZE; i++) {
        EXPECT_TRUE(rb1.contains(data[i]));
    }
    ...
    EXPECT_TRUE(nHits > 25);
    EXPECT_TRUE(nHits < 175);

    EXPECT_TRUE(rb1.contains(data[DATASIZE-1]));
    rb1.reset();
    EXPECT_TRUE(!rb1.contains(data[DATASIZE-1]));
    ...
    for (int i = 0; i < DATASIZE; i++) {
        if (i >= 100)
            EXPECT_TRUE(rb1.contains(data[i-100]));
        rb1.insert(data[i]);
    }
    ...
    GTEST_COUT << "RollingBloomFilter got " << nHits << " false positives (~5 expected)" << endl;
    EXPECT_TRUE(nHits < 100);
