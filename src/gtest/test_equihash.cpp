...
#define GTEST_TEST_MESSAGE cout << "[          ] "

void PrintSolution(stringstream &strm, v_uint32 soln) {
    strm << "  {";
...
void PrintSolutions(stringstream &strm, set<v_uint32> solns) {
    strm << "{";
    const char* soln_separator = "";
    for (v_uint32 soln : solns) {
        strm << soln_separator << "\n";
        soln_separator = ",";
...
#ifdef ENABLE_MINING
void TestEquihashSolvers(unsigned int n, unsigned int k, const string &I, const arith_uint256 &nonce, const set<v_uints> &solns) {
    size_t cBitLen { n/(k+1) };
    crypto_generichash_blake2b_state state;
    EhInitialiseState(n, k, state);
    uint256 V = ArithToUint256(nonce);
    // GTEST_TEST_MESSAGE << "Running solver: n = " << n << ", k = " << k << ", I = " << I << ", V = " << V.GetHex() << endl;
    ...
    // GTEST_TEST_MESSAGE << "[Basic] Number of solutions: " << ret.size() << endl;
    stringstream strm;
    PrintSolutions(strm, ret);
    // GTEST_TEST_MESSAGE << strm.str() << endl;
    EXPECT_EQ(ret , solns);

    // The optimised solver should have the exact same result
    set<v_uint32> retOpt;
    function<bool(const v_uint8 &)> validBlockOpt =
    ...
    // GTEST_TEST_MESSAGE << "[Optimised] Number of solutions: " << retOpt.size() << endl;
    strm.str("");
    PrintSolutions(strm, retOpt);
    // GTEST_TEST_MESSAGE << strm.str() << endl;
    EXPECT_EQ(retOpt , solns);
    EXPECT_EQ(retOpt , ret);
}
#endif

void TestEquihashValidator(unsigned int n, unsigned int k, const string &I, const arith_uint256 &nonce, const v_uints &soln, bool expected) {
    size_t cBitLen { n/(k+1) };
    crypto_generichash_blake2b_state state;
    EhInitialiseState(n, k, state);
    uint256 V = ArithToUint256(nonce);
    crypto_generichash_blake2b_update(&state, (unsigned char*)&I[0], I.size());
    crypto_generichash_blake2b_update(&state, V.begin(), V.size());
    // GTEST_TEST_MESSAGE << "Running validator: n = " << n << ", k = " << k << ", I = " << I << ", V = " << V.GetHex() << ", expected = " << expected << ", soln =" << endl;
    stringstream strm;
    PrintSolution(strm, soln);
    // GTEST_TEST_MESSAGE << strm.str() << endl;
    bool isValid;
...
void TestExpandAndCompress(const string &scope, size_t bit_len, size_t byte_pad,
                           v_uint8 compact,
                           v_uint8 expanded)
{
    SCOPED_TRACE(scope);

    v_uint8 out(expanded.size());
...
void TestMinimalSolnRepr(const string &scope, size_t cBitLen,
                         vector<eh_index> indices,
                         v_uint8 minimal)
{
    SCOPED_TRACE(scope);
...
TEST(equihash_tests, is_probably_duplicate) {
    shared_ptr<eh_trunc> p1 (new eh_trunc[4] {0, 1, 2, 3}, default_delete<eh_trunc[]>());
    shared_ptr<eh_trunc> p2 (new eh_trunc[4] {0, 1, 1, 3}, default_delete<eh_trunc[]>());
    shared_ptr<eh_trunc> p3 (new eh_trunc[4] {3, 1, 1, 3}, default_delete<eh_trunc[]>());
...
TEST(equihash_tests, check_basic_solver_cancelled) {
    ...
    {
        ASSERT_NO_THROW(Eh48_5.BasicSolve(state, [](v_uint8 soln) {
            return false;
    ...
    {
        ASSERT_THROW(Eh48_5.BasicSolve(state, [](v_uint8 soln) {
            return false;
        }, [](EhSolverCancelCheck pos) {
...
TEST(equihash_tests, check_optimised_solver_cancelled) {
    ...
    {
        ASSERT_NO_THROW(Eh48_5.OptimisedSolve(state, [](v_uint8 soln) {
            return false;
        }, [](EhSolverCancelCheck pos) {
            return false;
        }));
    ...
    {
        ASSERT_THROW(Eh48_5.OptimisedSolve(state, [](v_uint8 soln) {
            return false;
        }, [](EhSolverCancelCheck pos) {
            return pos == ListGeneration;
        }), EhSolverCancelledException);
...
TEST(equihash_tests, solver_testvectors)
{
...
TEST(equihash_tests, validator_testvectors)
{
