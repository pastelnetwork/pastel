...
unsigned int CScriptCompressor::GetSpecialSize(const unsigned int nSize) const noexcept
{
    if (nSize == 0 || nSize == 1)
        return 20;
    if (nSize >= 2 && nSize <= 5)
...
bool CScriptCompressor::Decompress(const unsigned int nSize, const v_uint8& in)
{
    switch(nSize) {
    case 0x00:
        script.resize(25);
