...
def store_data_in_animecoin_blockchain_UTXO_func(input_data, animecoin_zstd_compression_dictionary):
    ...
    compressed_file_size_in_bytes = sys.getsizeof(animecoin_zstd_compressed_data)
    ...
    assert(input_data == animecoin_zstd_uncompressed_data) #Check that we can decompress successfully
...
def retrieve_data_from_animecoin_blockchain_UTXO_func(blockchain_transaction_id):
    global rpc_connection_string
    global path_where_animecoin_html_ticket_templates_are_stored
    global path_to_zstd_compression_dictionaries
    rpc_connection = AuthServiceProxy(rpc_connection_string)
...
if use_demonstrate_blockchain_UTXO_storage: 
    ...
    list_of_available_compression_dictionary_files = glob.glob(path_to_zstd_compression_dictionaries + '*.dict')
    if len(list_of_available_compression_dictionary_files) == 0:
        animecoin_zstd_compression_dictionary = generate_zstd_dictionary_from_folder_path_and_file_matching_string_func(path_where_animecoin_html_ticket_templates_are_stored, '*.html')
