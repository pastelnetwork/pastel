...
from pastel_utility_functions_v1 import get_sha256_hash_of_input_data_func, get_all_pastel_directories_func, get_all_pastel_parameters_func, get_example_keypair_of_rsa_public_and_private_keys_func, get_example_keypair_of_rsa_public_and_private_keys_pem_format_func, \
    generate_required_directories_func, regenerate_sqlite_chunk_database_func, get_local_masternode_pastel_id_keypair_func, get_my_local_ip_func, generate_and_save_local_masternode_identification_keypair_func, get_various_directories_for_testing_func, \
    ...
    get_avg_pastel_mining_difficulty_rate_first_20k_blocks_func, get_current_datetime_string_func, parse_datetime_string_func, generate_recent_random_datetime_string_func, get_image_deep_learning_features_func
from pastel_image_processing_v1 import check_if_image_is_likely_dupe_func, add_all_images_in_folder_to_image_fingerprint_database_func, apply_tsne_to_image_fingerprint_database_func, \
...
from pastel_fountain_coding_v1 import refresh_block_storage_folder_and_check_block_integrity_func, decode_block_files_into_art_zipfile_func, randomly_delete_percentage_of_local_block_files_func, \
    randomly_corrupt_a_percentage_of_bytes_in_a_random_sample_of_block_files_func
from pastel_art_registration_v1 import register_new_artwork_folder_func, generate_combined_image_and_metadata_hash_func, get_concatenated_art_image_file_hashes_and_total_size_in_mb_func, create_metadata_html_table_for_given_art_folder_func, \
    perform_superficial_validation_of_html_metadata_table_func, validate_folder_for_nsfw_and_dupe_content_func
from pastel_trading_v1 import sign_trade_ticket_hash_func, verify_trade_ticket_hash_signature_func, generate_trade_ticket_func
###############################################################################################################
# Parameters:
###############################################################################################################
...
dupe_detection_model_3_name, path_to_nsfw_detection_model_file = get_all_pastel_parameters_func()
...
prepared_final_art_zipfiles_folder_path, chunk_db_file_path, file_storage_log_file_path, nginx_allowed_ip_whitelist_file_path, path_to_pastel_trade_ticket_template, \
artist_final_signature_files_folder_path, dupe_detection_image_fingerprint_database_file_path, path_to_pastel_artwork_metadata_template, path_where_pastel_html_ticket_templates_are_stored = get_all_pastel_directories_func()
...
use_generate_local_masternode_pastel_public_id_keypair = 0
...
if use_generate_local_masternode_pastel_public_id_keypair:
...
if use_demo_mode:
    print('\nWelcome! This is a demo of the file storage system that is being used for the Pastel project to store art files in a decentralized, robust way.')    
...
if use_demonstrate_duplicate_detection:
    ...
    add_all_images_in_folder_to_image_fingerprint_database_func('C:\\pastel_image_database\\')
    list_of_image_sha256_hashes, _, _, _, _, _, _ = apply_tsne_to_image_fingerprint_database_func()
    dupe_detection_test_images_base_folder_path = 'C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\dupe_detector_test_images\\' #Stress testing with sophisticated "modified" duplicates:
...
if use_demonstrate_combined_metadata_hash_generation:
    ...
    artists_pastel_id_public_key_pem_format = example_artist_public_key_pem_format
    artists_receiving_pastel_blockchain_address = example_artists_receiving_blockchain_address
    computed_combined_image_and_metadata_hash, combined_image_and_metadata = generate_combined_image_and_metadata_hash_func(concatenated_image_file_hashes, combined_size_of_artwork_image_files_in_megabytes, artists_name, artists_pastel_id_public_key_pem_format, artists_receiving_pastel_blockchain_address, artwork_title, total_number_of_unique_copies_of_artwork, artwork_series_name, artists_website, artists_statement_about_artwork)
...
if use_demonstrate_trade_ticket_generation:
    submitting_trader_pastel_blockchain_address = example_trader_blockchain_address
    desired_trade_type = 'BUY'
    desired_art_asset_hash = '570f4b835404a83e9870b9e98a0e2ca3b58b43c775cf1275a6690ab618f0e73c'
    desired_quantity = 1
    specified_price_in_pastel = 75000
    submitting_trader_pastel_identity_public_key = example_artist_public_key
    submitting_trader_pastel_identity_private_key = example_artist_private_key
    ...
    trade_ticket_details_sha256_hash, submitting_traders_digital_signature_on_trade_ticket_details_hash = sign_trade_ticket_hash_func(submitting_trader_pastel_identity_private_key, submitting_trader_pastel_identity_public_key, submitting_trader_pastel_blockchain_address, assigned_masternode_broker_ip_address, assigned_masternode_broker_pastel_identity_public_key, desired_trade_type, desired_art_asset_hash, desired_quantity, specified_price_in_pastel)#This would happen on the artist/trader's machine.
    trade_ticket_signature_is_valid = verify_trade_ticket_hash_signature_func(trade_ticket_details_sha256_hash, submitting_traders_digital_signature_on_trade_ticket_details_hash, submitting_trader_pastel_identity_public_key)
    trade_ticket_output_file_path = generate_trade_ticket_func(submitting_trader_pastel_identity_public_key,
                                                               submitting_trader_pastel_blockchain_address,
                                                               desired_trade_type,
