...
import shortuuid
import rsa
from pastel_utility_functions_v1 import download_remote_file_func, get_all_remote_node_ips_func, get_my_local_ip_func, get_local_masternode_identification_keypair_func
from pastel_file_transfer_v1 import get_list_of_available_files_on_remote_node_file_server_func
...
def generate_trade_ticket_func(submitting_trader_pastel_identity_public_key, submitting_trader_pastel_blockchain_address, desired_trade_type, desired_art_asset_hash, desired_quantity, specified_price_in_pastel, submitting_traders_digital_signature_on_trade_ticket_details_hash):
    global example_pastel_masternode_blockchain_address
    global path_to_pastel_trade_ticket_template
    global example_masternode_public_key
    global example_masternode_private_key
    global trade_ticket_files_folder_path
    global pending_trade_ticket_files_folder_path
    with open(path_to_pastel_trade_ticket_template,'r') as f:
    ...
    specified_price_in_pastel_formatted = locale.format('%f', specified_price_in_pastel, grouping=True)
    submitting_trader_pastel_identity_public_key_pem_format = submitting_trader_pastel_identity_public_key.save_pkcs1(format='PEM').decode('utf-8')
    assigned_masternode_broker_ip_address = get_my_local_ip_func()
    assigned_masternode_broker_pastel_identity_public_key, assigned_masternode_broker_pastel_identity_private_key = get_local_masternode_identification_keypair_func()
    assigned_masternode_broker_pastel_identity_public_key_pem_format = assigned_masternode_broker_pastel_identity_public_key.save_pkcs1(format='PEM')
    assigned_masternode_broker_pastel_blockchain_address = example_pastel_masternode_blockchain_address
    trade_ticket_details_sha256_hash, assigned_masternode_broker_digital_signature_on_trade_ticket_details_hash_base64_encoded = sign_trade_ticket_hash_func(assigned_masternode_broker_pastel_identity_private_key, assigned_masternode_broker_pastel_identity_public_key_pem_format, submitting_trader_pastel_blockchain_address, assigned_masternode_broker_ip_address, assigned_masternode_broker_pastel_identity_public_key, desired_trade_type, desired_art_asset_hash, desired_quantity, specified_price_in_pastel)#This would happen on the artist/trader's machine.
    ...
    x = x.replace('TRADE_TRADER_PUBLIC_KEY_ID_VAR', submitting_trader_pastel_identity_public_key_pem_format)
    x = x.replace('TRADE_TRADER_PASTEL_ADDRESS_VAR', submitting_trader_pastel_blockchain_address)
    ...
    x = x.replace('TRADE_PRICE_VAR', specified_price_in_pastel_formatted)
    ...
    x = x.replace('TRADE_ASSIGNED_BROKER_PASTEL_ADDRESS_VAR', assigned_masternode_broker_pastel_blockchain_address)
    x = x.replace('TRADE_ASSIGNED_BROKER_PUBLIC_KEY_ID_VAR', assigned_masternode_broker_pastel_identity_public_key_pem_format.decode('utf-8'))
    ...
    trade_ticket_output_file_path = os.path.join(pending_trade_ticket_files_folder_path,'Pastel_Trade_Ticket__TradeID__' + new_trade_id + '__DateTime_Submitted__'+datetime_trade_submitted.replace(':','_').replace('-','_').replace(' ','_') + '.html')
...
def sign_trade_ticket_hash_func(signer_private_key, submitting_trader_pastel_identity_public_key, submitting_trader_pastel_blockchain_address, assigned_masternode_broker_ip_address, assigned_masternode_broker_pastel_identity_public_key, desired_trade_type, desired_art_asset_hash, desired_quantity, specified_price_in_pastel):
    desired_quantity_formatted = locale.format('%d', desired_quantity, grouping=True)
    specified_price_in_pastel_formatted = locale.format('%f', specified_price_in_pastel, grouping=True)
    submitting_trader_pastel_identity_public_key_pem_format = submitting_trader_pastel_identity_public_key.save_pkcs1(format='PEM').decode('utf-8')
    assigned_masternode_broker_pastel_identity_public_key_pem_format = assigned_masternode_broker_pastel_identity_public_key.save_pkcs1(format='PEM').decode('utf-8')
    data_for_trade_ticket_hash = submitting_trader_pastel_identity_public_key_pem_format + submitting_trader_pastel_blockchain_address +  assigned_masternode_broker_ip_address + assigned_masternode_broker_pastel_identity_public_key_pem_format + desired_trade_type + desired_art_asset_hash + desired_quantity_formatted + specified_price_in_pastel_formatted
...
def parse_trade_ticket_func(path_to_trade_ticket_html_file):
    ...
    submitting_trader_pastel_blockchain_address = parsed_trade_data_list[2]
    submitting_trader_pastel_identity_public_key_pem_format = parsed_trade_data_list[3]
    submitting_trader_pastel_identity_public_key = rsa.PublicKey.load_pkcs1(submitting_trader_pastel_identity_public_key_pem_format,format='PEM')
    ...
    specified_price_in_pastel = locale.atof(parsed_trade_data_list[7])
    ...
    assigned_masternode_broker_pastel_blockchain_address = parsed_trade_data_list[11]
    assigned_masternode_broker_pastel_identity_public_key_pem_format = parsed_trade_data_list[12]
    assigned_masternode_broker_pastel_identity_public_key = rsa.PublicKey.load_pkcs1(assigned_masternode_broker_pastel_identity_public_key_pem_format,format='PEM')
    ...
    return trade_id, datetime_trade_submitted, submitting_trader_pastel_blockchain_address, submitting_trader_pastel_identity_public_key, desired_trade_type, desired_art_asset_hash, desired_quantity, specified_price_in_pastel, trade_ticket_details_sha256_hash, submitting_traders_digital_signature_on_trade_ticket_details_hash, assigned_masternode_broker_ip_address, assigned_masternode_broker_pastel_blockchain_address, assigned_masternode_broker_pastel_identity_public_key, assigned_masternode_broker_digital_signature_on_trade_ticket_details_hash

def verify_signatures_on_trade_ticket_func(path_to_trade_ticket_html_file):
    all_verified = 0
    trade_id, datetime_trade_submitted, submitting_trader_pastel_blockchain_address, submitting_trader_pastel_identity_public_key, desired_trade_type, desired_art_asset_hash, desired_quantity, specified_price_in_pastel, trade_ticket_details_sha256_hash, submitting_traders_digital_signature_on_trade_ticket_details_hash, assigned_masternode_broker_ip_address, assigned_masternode_broker_pastel_blockchain_address, assigned_masternode_broker_pastel_identity_public_key, assigned_masternode_broker_digital_signature_on_trade_ticket_details_hash = parse_trade_ticket_func(path_to_trade_ticket_html_file)
    desired_quantity_formatted = locale.format('%d', desired_quantity, grouping=True)
    specified_price_in_pastel_formatted = locale.format('%f', specified_price_in_pastel, grouping=True)
    submitting_trader_pastel_identity_public_key_pem_format = submitting_trader_pastel_identity_public_key.save_pkcs1(format='PEM').decode('utf-8')    
    assigned_masternode_broker_pastel_identity_public_key_pem_format = assigned_masternode_broker_pastel_identity_public_key.save_pkcs1(format='PEM').decode('utf-8')
    ...
    if computed_trade_ticket_details_sha256_hash == trade_ticket_details_sha256_hash:
        ...
        submitting_trader_signatured_was_verified = verify_trade_ticket_hash_signature_func(trade_ticket_details_sha256_hash, submitting_traders_digital_signature_on_trade_ticket_details_hash, submitting_trader_pastel_identity_public_key)
        ...
        assigned_masternode_broker_signatured_was_verified = verify_trade_ticket_hash_signature_func(trade_ticket_details_sha256_hash, assigned_masternode_broker_digital_signature_on_trade_ticket_details_hash, assigned_masternode_broker_pastel_identity_public_key)
...
def write_pending_trade_ticket_to_chunkdb_func(path_to_trade_ticket_html_file):
    ...
    try:
        ...
        trade_ticket_details_sha256_hash, 
        submitting_trader_pastel_identity_public_key,
        ...
        c.execute(update_table_data_query_string, [desired_art_asset_hash,
                                                  trade_ticket_details_sha256_hash, 
                                                  submitting_trader_pastel_identity_public_key_pem_format,
                                                  submitting_traders_digital_signature_on_trade_ticket_details_hash_base64_encoded,
                                                  submitting_trader_pastel_blockchain_address,
                                                  assigned_masternode_broker_ip_address,
                                                  assigned_masternode_broker_pastel_identity_public_key_pem_format,
                                                  assigned_masternode_broker_pastel_blockchain_address,
                                                  assigned_masternode_broker_digital_signature_on_trade_ticket_details_hash_base64_encoded,
                                                  desired_trade_type,
                                                  desired_quantity,
