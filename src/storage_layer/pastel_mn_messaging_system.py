...
import nacl
import zstd
from pastel_pastel_id_self_contained_demo import import_pastel_public_and_private_keys_from_pem_files_func, pastel_id_keypair_generation_func, \
    write_pastel_public_and_private_key_to_file_func, pastel_id_write_signature_on_data_func, pastel_id_verify_signature_with_public_key_func
...
class messengerObject(object):
    ...
    def __init__(self, receiver_id, message_body):
        ...
        sleep_rand()
        use_require_otp = 0
        pastel_id_public_key_b16_encoded, pastel_id_private_key_b16_encoded = import_pastel_public_and_private_keys_from_pem_files_func(use_require_otp)
        if pastel_id_public_key_b16_encoded == '':
            ...
            write_pastel_public_and_private_key_to_file_func(pastel_id_public_key_b16_encoded, pastel_id_private_key_b16_encoded)
        self.pastel_id_public_key_b16_encoded = pastel_id_public_key_b16_encoded
        self.pastel_id_private_key_b16_encoded = pastel_id_private_key_b16_encoded
        self.sender_id = pastel_id_public_key_b16_encoded
    ...
    def sign_combined_raw_message(self):
        sleep_rand()
        self.signature_on_raw_message = pastel_id_write_signature_on_data_func(self.hash_of_combined_message_string, self.pastel_id_private_key_b16_encoded, self.pastel_id_public_key_b16_encoded)
        signature_string = '\n\ndigital_signature: \n'
    ...
    def sign_compressed_message(self):
        sleep_rand()
        self.signature_on_compressed_message = pastel_id_write_signature_on_data_func(self.hash_of_compressed_signed_combined_message, self.pastel_id_private_key_b16_encoded, self.pastel_id_public_key_b16_encoded)
        sleep_rand()
        return self
            ...
            if len(raw_message_contents) < 4000:
                ...
                for current_field in message_contents_fields[1:-1]:
                    ...
                    if current_field[:len(sender_string)] == sender_string:
                        senders_pastel_id = current_field.replace(sender_string,'').replace('\n','')
                        assert(len(senders_pastel_id)==132)
                        assert([(x in id_character_set) for x in senders_pastel_id])
                    receiver_string = '\n\nreceiver_id:\n'
                    assert(len(message_contents.split(receiver_string))==2)
                    if current_field[:len(receiver_string)]==receiver_string:
                        receivers_pastel_id = current_field.replace(receiver_string,'').replace('\n','')
                        assert(len(receivers_pastel_id)==132)
                        assert([(x in id_character_set) for x in receivers_pastel_id])
                        assert(senders_pastel_id!=receivers_pastel_id)
                    timestamp_string = '\n\ntimestamp:\n'
                    ...
                    sleep_rand()
                verified = pastel_id_verify_signature_with_public_key_func(hash_of_combined_message_string, signature_line, senders_pastel_id)
                sleep_rand()
                return verified, senders_pastel_id, receivers_pastel_id, timestamp_of_message, message_size, message_body, random_nonce, message_contents, signature_line
    ...
    def verify_compressed_message_file(self, senders_pastel_id, compressed_binary_data, signature_on_compressed_file):
        if isinstance(compressed_binary_data, bytes):
            if sys.getsizeof(compressed_binary_data) < 2800:
                ...
                if len(senders_pastel_id) == 132:
                    sleep_rand()
                    verified = pastel_id_verify_signature_with_public_key_func(hash_of_compressed_message, signature_on_compressed_file, senders_pastel_id)
                    sleep_rand()
                    if verified:
                        try:
...
if 'test_receiver__pastel_id_private_key_b16_encoded' not in globals():
    test_receiver__pastel_id_private_key_b16_encoded, test_receiver__pastel_id_public_key_b16_encoded = pastel_id_keypair_generation_func()
receiver_id = test_receiver__pastel_id_public_key_b16_encoded
...
with open(path_of_most_recent_raw_message_file, 'r') as f:
    try:
        ...
        print('Error: '+ str(e))
verified, senders_pastel_id, receivers_pastel_id, timestamp_of_message, message_size, message_body, random_nonce, message_contents, signature_line = messenger_object.verify_raw_message_file(raw_message_contents)
...
use_require_otp = 0
senders_pastel_id, _ = import_pastel_public_and_private_keys_from_pem_files_func(use_require_otp)
decompressed_message_data = messenger_object.verify_compressed_message_file(senders_pastel_id, compressed_message_contents, compressed_signature)
verified, senders_pastel_id, receivers_pastel_id, timestamp_of_message, message_size, message_body, random_nonce, message_contents, signature_line = messenger_object.verify_raw_message_file(decompressed_message_data)
if verified:
