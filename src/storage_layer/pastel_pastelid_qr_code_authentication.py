...
import pyqrcode
from pastel_pastel_id_self_contained_demo import pastel_id_keypair_generation_func
...
def generate_qr_code_from_pastel_id_privkey_func(pastel_id_private_key_b16_encoded):
    assert(isinstance(pastel_id_private_key_b16_encoded , str))
    assert(len(pastel_id_private_key_b16_encoded)==2084)
    key_character_set = 'ABCDEF1234567890'
    assert([(x in key_character_set) for x in pastel_id_private_key_b16_encoded])
    print('Generating Video of QR Codes now...')
    number_of_key_chunks = 25
    key_chunk_size = int(math.ceil(2084/number_of_key_chunks))
    key_chunks = [str(int(math.ceil(ii/key_chunk_size)+1)) + ' ' + str(number_of_key_chunks) + ' ' + pastel_id_private_key_b16_encoded[ii:ii+key_chunk_size] for ii in range(0, len(pastel_id_private_key_b16_encoded), key_chunk_size)]
    ...
    for cnt, current_key_chunk in enumerate(key_chunks_duplicated):
        ...
        warning_message_1 = "Warning! Once you close this window,  these QR codes will be lost! The video of QR codes below represents your Pastel ID Private Key-- make sure it stays secret!"
        ...
        warning_message_4 = "Then, you will be able to unlock your Pastel wallet by holding up your phone to your computer's web cam while the video plays on your phone, which is convenient and secure."
...
if use_demonstrate_qr_key_video_generation:
    ...
    print('Pastel ID Private Key:\n' + pastel_id_private_key_b16_encoded)
    generate_qr_code_from_pastel_id_privkey_func(pastel_id_private_key_b16_encoded)
...
if use_demonstrate_qr_key_recognition:
    ...
    if 0:
        assert(reconstructed_key_string==pastel_id_private_key_b16_encoded)
