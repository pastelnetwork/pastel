import os, time, base64, hashlib, glob, random, sys, io, struct, subprocess, math
...
def compress_data_with_zstd_func(input_data, zstd_compression_level):
    zstandard_compressor = zstd.ZstdCompressor(level=zstd_compression_level, write_content_size=True) #Highest (best) compression level is 22
