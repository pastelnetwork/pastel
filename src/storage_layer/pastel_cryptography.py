...
from pastel_utility_functions_v1 import get_sha256_hash_of_input_data_func
from pastel_fountain_coding_v1 import PRNG, BlockGraph\
...
def encode_file_into_ztd_luby_block_qr_codes_func(input_data_or_path): #sphincs_secret_key_raw_bytes
    ...
    for current_block_data in output_blocks_list:
        ...
        for cnt in range(required_number_of_qr_codes):
            ...
            qr_code_png_output_file_name = 'PASTEL_QR_CODE__'+ str(compressed_input_data_hash) + '__' '{0:05}'.format(overall_counter)
    ...
    combined_qr_code_png_output_file_name = 'PASTEL_QR_CODE_COMBINED__' + compressed_input_data_hash + '.png'
...
def generate_data_animation_func(input_data_or_path):
    ...
    for cnt, current_qr_code_image_data in enumerate(list_of_qr_code_image_data):
       current_output_file_name = 'PASTEL_QR_ANIMATION__Frame_'+'{0:03}'.format(cnt) + '.png'
...
if use_demonstrate_sphincs_crypto:
    ...
    with MyTimer():
        ...
        path_to_pastel_html_ticket_file = 'C:\\pastel\\art_folders_to_encode\\Carlo_Angelo__Felipe_Number_02\\artwork_metadata_ticket__2018_05_27__23_42_33_964953.html'
        with open(path_to_pastel_html_ticket_file,'rb') as f:
           pastel_ticket_html_string = f.read()
        message = pastel_ticket_html_string
...
if use_demonstrate_qr_data_transmission_animation:
    existing_frame_paths = glob.glob('PASTEL_QR_ANIMATION__Frame_*.png')
    ...
    if 0: #make into animated gif:
        gif_name = combined_qr_code_png_output_file_name.replace('.png','').replace('PASTEL_QR_CODE_COMBINED__','PASTEL_QR_ANIMATION__')
        fps = 3
        file_list = glob.glob('PASTEL_QR_ANIMATION__Frame_*.png') # Get all the pngs in the current directory
        clip = mpy.ImageSequenceClip(file_list, fps=fps)
        clip.write_gif('{}.gif'.format(gif_name), fps=fps)
        animation_gif_file_paths = glob.glob('PASTEL_QR_ANIMATION__*.gif')
    ...
    player_uri_string = 'file:///C:/Users/jeffr/Cointel%20Dropbox/Pastel_Code/'+animated_qr_player_html_output_file_path
    for cnt, current_svg_blob in enumerate(list_of_qr_code_svg_blobs):
        animated_qr_code_file_output_file_path = 'PASTEL_QR_ANIMATION__Frame_'+'{0:04}'.format(cnt) +'.png'
...
if use_demonstrate_qr_animation_video_recovery:
    ...
    video_frame_paths = glob.glob('C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\video_frames\\*.jpg')
    for current_frame_path in video_frame_paths:
        os.remove(current_frame_path)
    while success:
      success, image_frame = vidcap.read()
      if count > skip_first_n_frames:
          cv2.imwrite('C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\video_frames\\frame__'+'{0:04}'.format(count) +'.png', image_frame)
      count = count + 1
    video_frame_paths = glob.glob('C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\video_frames\\*.jpg')
    for current_frame_path in video_frame_paths:
...
#        cv2.imwrite('C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\video_frames\\block_detctor__' + str(cnt) + '.png', im_with_keypoints)
...
#                cv2.imwrite('C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\video_frames\\contour__' + str(cpt) + '.png', img2)
#                cpt = cpt + 1
#
...
if use_demonstrate_qr_steganography:
    path_to_image_file = 'C:\\pastel\\art_folders_to_encode\\Carlo_Angelo__Felipe_Number_02\\Carlo_Angelo__Felipe_Number_02.png'
    path_to_image_file_with_qr_codes = glob.glob('PASTEL_QR_CODE_COMBINED__*.png')
    ...
    merged_image_data_png_output_file_name = 'PASTEL_ART_Watermarked.png'
    ...
    embedded_image_data_png_output_file_name = 'PASTEL_ART_SIGNATURE.png'
    embedded_image.save(embedded_image_data_png_output_file_name)
    path_to_reconstructed_image_file_with_qr_codes = 'PASTEL_ART_SIGNATURE.png'
