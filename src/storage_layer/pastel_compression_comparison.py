...
from pastel_load_various_ticket_template_files_and_executed_tickets import generate_pastel_html_strings_for_templates_and_example_executed_tickets_func, convert_list_of_strings_into_list_of_utf_encoded_byte_objects_func
...
if use_large_binary_file_instead_of_html_input_file:
    path_to_folder_containing_files_for_dictionary = 'C:\\pastel\\art_block_storage\\'
    file_matching_string = '*3c2ffc31e848e9f9bbb63d9a2890d5a7c89c2110e5d502ea48d05f1c993b17e8*'
    pastel_zstd_compression_dictionary = generate_zstd_dictionary_from_folder_path_and_file_matching_string_func(path_to_folder_containing_files_for_dictionary, file_matching_string)

    block_file_path = 'C:\\pastel\\art_block_storage\\FileHash__fbb3ad3d0e77bd707650a86f828fd8e26972a1ed98a7964bc378b2ea39cf8e7e__Block__000000012__BlockHash_314772985be4b62ea281a70ebe59ae414a34ab5ca18200ca70364ec3747688ad.block'
    with open(block_file_path,'rb') as f:
        sample_input_data = f.read()
else:
    path_to_folder_containing_files_for_dictionary = 'C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\pastel_html_ticket_template_files\\'
    file_matching_string = '*.html'
    pastel_zstd_compression_dictionary = generate_zstd_dictionary_from_folder_path_and_file_matching_string_func(path_to_folder_containing_files_for_dictionary, file_matching_string)
    _, example_executed_artwork_metadata_html_ticket_string = generate_pastel_html_strings_for_templates_and_example_executed_tickets_func()
    sample_input_data = example_executed_artwork_metadata_html_ticket_string.encode('utf-8')
    #pastel_zstd_compression_dictionary = zstd.ZstdCompressionDict(concatenated_list_of_pastel_html_strings_for_templates_and_example_executed_tickets_byte_encoded)

print('Running each compressor and decompressor on input data...')
uncompressed_size_in_bytes = sys.getsizeof(sample_input_data)

pastel_zstd_compressed_data = compress_data_with_pastel_zstd_func(sample_input_data, pastel_zstd_compression_dictionary)
...
if pastel_zstd_uncompressed_data == sample_input_data:
    print('Pastel ZSTD Archive decompressed successfully!')
...
pastel_zstd_compressed_size_in_bytes = sys.getsizeof(pastel_zstd_compressed_data)
...
pastel_zstd_size_as_pct_of_bz2_size = pastel_zstd_compressed_size_in_bytes/bz2_compressed_size_in_bytes
pastel_zstd_size_as_pct_of_gzip_size = pastel_zstd_compressed_size_in_bytes/gzip_compressed_size_in_bytes
pastel_zstd_size_as_pct_of_lzma_size = pastel_zstd_compressed_size_in_bytes/lzma_compressed_size_in_bytes
pastel_zstd_size_as_pct_of_paq_size = pastel_zstd_compressed_size_in_bytes/paq_compressed_size_in_bytes

print('Uncompressed size: '+str(uncompressed_size_in_bytes))
print('PAQ Compressed size: '+str(paq_compressed_size_in_bytes))
print('Pastel ZSTD Compressed size: '+str(pastel_zstd_compressed_size_in_bytes))
...
print('Pastel ZSTD size as % of BZ2 Size: '+str(pastel_zstd_size_as_pct_of_bz2_size))
print('Pastel ZSTD size as % of GZIP Size: '+str(pastel_zstd_size_as_pct_of_gzip_size))
print('Pastel ZSTD size as % of LZMA Size: '+str(pastel_zstd_size_as_pct_of_lzma_size))
print('Pastel ZSTD size as % of PAQ Size: '+str(pastel_zstd_size_as_pct_of_paq_size))
...
"""
    ...
    print('Pastel ZSTD: Now compressing '+str(uncompressed_size_in_bytes)+' bytes of input data.')
    %timeit -r 3 pastel_zstd_compressed_data = compress_data_with_pastel_zstd_func(sample_input_data, pastel_zstd_compression_dictionary)
    ...
    print('Pastel ZSTD: Now decompressing data...')
    %timeit -r 3 pastel_zstd_uncompressed_data = decompress_data_with_pastel_zstd_func(pastel_zstd_compressed_data, pastel_zstd_compression_dictionary)
