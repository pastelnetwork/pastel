...
def get_pastel_root_directory_func():
    current_platform = platform.platform()
    if 'Windows' in current_platform:
        root_pastel_folder_path = 'C:\\pastel\\'
    else:
        root_pastel_folder_path = '/home/pasteluser/pastel/'
    return root_pastel_folder_path
...
def get_all_pastel_directories_func():
    root_pastel_folder_path = get_pastel_root_directory_func()
    folder_path_of_art_folders_to_encode = os.path.join(root_pastel_folder_path,'art_folders_to_encode' + os.sep)#Each subfolder contains the various art files pertaining to a given art asset.
    block_storage_folder_path = os.path.join(root_pastel_folder_path,'art_block_storage' + os.sep)
    folder_path_of_remote_node_sqlite_files = os.path.join(root_pastel_folder_path,'remote_node_sqlite_files' + os.sep)
    reconstructed_file_destination_folder_path = os.path.join(root_pastel_folder_path,'reconstructed_files' + os.sep)
    ...
    trade_ticket_files_folder_path = os.path.join(root_pastel_folder_path,'trade_ticket_files' + os.sep) #Where we store the html trade tickets for the masternode DEX
    ...
    temp_files_folder_path = os.path.join(root_pastel_folder_path,'temp_files' + os.sep)
    prepared_final_art_zipfiles_folder_path = os.path.join(root_pastel_folder_path,'prepared_final_art_zipfiles' + os.sep) 
    chunk_db_file_path = os.path.join(misc_masternode_files_folder_path,'pastel_chunkdb.sqlite')
    file_storage_log_file_path = os.path.join(misc_masternode_files_folder_path,'pastel_storage_log.txt')
    nginx_allowed_ip_whitelist_file_path = '/var/www/masternode_file_server/html/masternode_ip_whitelist.conf'
    path_to_pastel_trade_ticket_template = os.path.join(trade_ticket_files_folder_path,'pastel_trade_ticket_template.html')
    artist_final_signature_files_folder_path = os.path.join(root_pastel_folder_path,'art_signature_files' + os.sep)
    ...
    path_to_pastel_artwork_metadata_template = os.path.join(misc_masternode_files_folder_path,'pastel_metadata_template.html')
    path_where_pastel_html_ticket_templates_are_stored = 'C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\'
    path_to_nsfw_detection_model_file = os.path.join(misc_masternode_files_folder_path,'nsfw_trained_model.pb')
    return root_pastel_folder_path, folder_path_of_art_folders_to_encode, block_storage_folder_path, folder_path_of_remote_node_sqlite_files, reconstructed_file_destination_folder_path, \
...
def get_all_pastel_parameters_func():
    pastel_metadata_format_version_number = '1.00'
    ...
    registration_fee_pastel_per_megabyte_of_images_pre_difficulty_adjustment = 12
    ...
    example_pastel_masternode_blockchain_address = 'AdaAvFegJbBdH4fB8AiWw8Z4Ek75Xsja6i'
    ...
    return pastel_metadata_format_version_number, minimum_total_number_of_unique_copies_of_artwork, maximum_total_number_of_unique_copies_of_artwork, target_number_of_nodes_per_unique_block_hash, target_block_redundancy_factor, desired_block_size_in_bytes, \
            ...
            example_pastel_masternode_blockchain_address, example_trader_blockchain_address, example_artists_receiving_blockchain_address, rpc_connection_string, \
...
dupe_detection_model_3_name = get_all_pastel_parameters_func()
...
def get_various_directories_for_testing_func():
    ...
    path_to_another_different_art_image_file = 'C:\\pastel\\art_folders_to_encode\\Arturo_Lopez__Number_04\\Arturo_Lopez__Number_04.png'
    #path_to_artwork_metadata_file = os.path.join(path_to_art_folder,'artwork_metadata_file.db')
    path_to_all_registered_works_for_dupe_detection = 'C:\\Users\\jeffr\Cointel Dropbox\\Jeffrey Emanuel\\Pastel_All_Finished_Works\\'
...
def generate_and_save_example_rsa_keypair_files_func():
    ...
    output_file_path = os.path.join(root_pastel_folder_path,'example_rsa_keypairs_pickle_file.p')
    try:
...
def get_example_keypair_of_rsa_public_and_private_keys_func():
    root_pastel_folder_path = get_pastel_root_directory_func()
    example_rsa_keypairs_pickle_file_path = os.path.join(root_pastel_folder_path,'example_rsa_keypairs_pickle_file.p')
...
def get_metadata_file_path_from_art_folder_func(path_to_art_folder):
    path_to_metadata_html_table_file = glob.glob(path_to_art_folder + 'Pastel_Metadata_File__Combined_Hash__*.html')[0]
...
def check_current_pastel_blockchain_stats_func(): #Todo: make this a real function using the RPC interface
...
def generate_new_pastel_blockchain_address_func(): #Todo: replace this with a real function that uses the RPC interface
    ...
    generated_pastel_blockchain_address = 'A' + shortuuid.uuid()[0:16] + shortuuid.uuid()[0:17]
    return generated_pastel_blockchain_address
...
def get_current_pastel_metadata_format_version_number_func():
    current_pastel_metadata_format_version_number = '1.00'
    return current_pastel_metadata_format_version_number
...
def get_current_pastel_mining_difficulty_func(): #Todo: replace this with a real function that uses the RPC interface
    current_pastel_mining_difficulty_rate = 350
    return current_pastel_mining_difficulty_rate

def get_avg_pastel_mining_difficulty_rate_first_20k_blocks_func(): #Todo: replace this with a real function that uses the RPC interface
    avg_pastel_mining_difficulty_rate_first_20k_blocks = 100
    return avg_pastel_mining_difficulty_rate_first_20k_blocks
...
def check_if_pastel_blockchain_address_is_valid_func(potential_pastel_address_string): #Todo: make this a real function using the RPC interface
    length_of_address = len(potential_pastel_address_string)
...
def validate_list_of_pastel_blockchain_addresses(list_of_pastel_blockchain_addresses_to_verify):
    list_of_valid_pastel_blockchain_addresses = []
    list_of_invalid_pastel_blockchain_addresses = []
    for current_blockchain_address in list_of_pastel_blockchain_addresses_to_verify:
        address_is_valid = check_if_pastel_blockchain_address_is_valid_func(current_blockchain_address)
        if address_is_valid == 1:
            list_of_valid_pastel_blockchain_addresses.append(current_blockchain_address)
        else:
            list_of_invalid_pastel_blockchain_addresses.append(current_blockchain_address)
    return list_of_valid_pastel_blockchain_addresses, list_of_invalid_pastel_blockchain_addresses
...
def regenerate_sqlite_chunk_database_func():
    ...
    try:
        ...
        pending_trade_tickets_table_creation_string = """CREATE TABLE pending_trade_tickets_table (desired_art_asset_hash text, trade_ticket_details_sha256_hash text, submitting_trader_pastel_identity_public_key text, submitting_traders_digital_signature_on_trade_ticket_details_hash text, submitting_trader_pastel_blockchain_address text,  assigned_masternode_broker_ip_address text, assigned_masternode_broker_pastel_identity_public_key text, assigned_masternode_broker_pastel_blockchain_address text, assigned_masternode_broker_digital_signature_on_trade_ticket_details_hash text, desired_trade_type text, desired_quantity integer, specified_price_in_pastel real, tradeid text, datetime_trade_submitted text, datetime_trade_executed text, datetime_inserted TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, PRIMARY KEY (trade_ticket_details_sha256_hash));"""
        c.execute(pending_trade_tickets_table_creation_string)
        completed_trade_tickets_table_creation_string = """CREATE TABLE completed_trade_tickets_table (desired_art_asset_hash text, trade_ticket_details_sha256_hash text, submitting_trader_pastel_identity_public_key text, submitting_traders_digital_signature_on_trade_ticket_details_hash text, submitting_trader_pastel_blockchain_address text,  assigned_masternode_broker_ip_address text, assigned_masternode_broker_pastel_identity_public_key text, assigned_masternode_broker_pastel_blockchain_address text, assigned_masternode_broker_digital_signature_on_trade_ticket_details_hash text, desired_trade_type text, desired_quantity integer, specified_price_in_pastel real, tradeid text, datetime_trade_submitted text, datetime_trade_executed text, datetime_inserted TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, PRIMARY KEY (trade_ticket_details_sha256_hash));"""
...
def get_local_masternode_pastel_id_keypair_func():
    global masternode_keypair_db_file_path
    if not os.path.exists(masternode_keypair_db_file_path):
        ...
        get_local_masternode_pastel_id_keypair_func()
    try:
        conn = sqlite3.connect(masternode_keypair_db_file_path)
        c = conn.cursor()
...
def calculate_artwork_registration_fee_func(combined_size_of_artwork_image_files_in_megabytes):
    global registration_fee_pastel_per_megabyte_of_images_pre_difficulty_adjustment
    ...
    avg_pastel_mining_difficulty_rate_first_20k_blocks = get_avg_pastel_mining_difficulty_rate_first_20k_blocks_func()
    current_pastel_mining_difficulty_rate = get_current_pastel_mining_difficulty_func()
    pastel_mining_difficulty_adjustment_ratio = float(current_pastel_mining_difficulty_rate) / float(avg_pastel_mining_difficulty_rate_first_20k_blocks)
    registration_fee_in_pastel_pre_difficulty_adjustment = round(float(registration_fee_pastel_per_megabyte_of_images_pre_difficulty_adjustment)*float(combined_size_of_artwork_image_files_in_megabytes)*float(target_number_of_nodes_per_unique_block_hash)*float(target_block_redundancy_factor))
    registration_fee_in_pastel_post_difficulty_adjustment = round(float(registration_fee_in_pastel_pre_difficulty_adjustment) / float(pastel_mining_difficulty_adjustment_ratio))
    artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration = round(float(registration_fee_in_pastel_post_difficulty_adjustment)*float(forfeitable_deposit_to_initiate_registration_as_percentage_of_adjusted_registration_fee))
    return registration_fee_in_pastel_post_difficulty_adjustment, registration_fee_in_pastel_pre_difficulty_adjustment, pastel_mining_difficulty_adjustment_ratio, artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration
...
def generate_combined_artwork_image_and_metadata_string_and_hash_func(pastel_artwork_metadata_ticket_format_version_number,
                                                  date_time_artwork_prepared_by_artist,
                                                  current_bitcoin_block_number,
                                                  current_bitcoin_block_hash,
                                                  current_pastel_block_number,
                                                  current_pastel_block_hash,
                                                  artist_pastel_id_public_key,
                                                  ...
                                                  current_pastel_mining_difficulty_rate,
                                                  average_pastel_mining_difficulty_rate_first_20k_blocks,
                                                  pastel_mining_difficulty_adjustment_ratio,
                                                  required_registration_fee_in_pastel_terms_pre_difficulty_adjustment,
                                                  required_registration_fee_in_pastel_terms_post_difficulty_adjustment,
                                                  artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration):
    ...
    list_of_metadata_fields = [pastel_artwork_metadata_ticket_format_version_number, date_time_artwork_prepared_by_artist, current_bitcoin_block_number, current_bitcoin_block_hash, current_pastel_block_number, current_pastel_block_hash, \
                                artist_pastel_id_public_key, str(total_number_of_unique_copies_of_artwork_to_create), artwork_title, artist_name, artwork_series_name, artist_website, artist_statement, combined_image_size_in_mb,  \
                                artwork_concatenated_image_hashes, str(current_pastel_mining_difficulty_rate), str(average_pastel_mining_difficulty_rate_first_20k_blocks), str(pastel_mining_difficulty_adjustment_ratio), \
                                str(required_registration_fee_in_pastel_terms_pre_difficulty_adjustment), str(required_registration_fee_in_pastel_terms_post_difficulty_adjustment), str(artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration)]
...
class fake_artwork_metadata_class(object):
       def __init__(self):
        global dupe_detection_model_1_name, dupe_detection_model_2_name, dupe_detection_model_3_name, path_where_pastel_html_ticket_templates_are_stored
        _, _, _, _, example_pastel_public_id_public_key, example_pastel_public_id_private_key = get_example_keypair_of_rsa_public_and_private_keys_func()
        _, _, _, _, example_pastel_public_id_public_key_pem_format, pastel_public_id_private_key_pem_format = get_example_keypair_of_rsa_public_and_private_keys_pem_format_func()
        root_pastel_folder_path = get_pastel_root_directory_func()
        folder_path_of_art_folders_to_encode = os.path.join(root_pastel_folder_path,'art_folders_to_encode' + os.sep)
        ...
        pastel_artwork_metadata_ticket_format_version_number = get_current_pastel_metadata_format_version_number_func()
        ...
        current_pastel_block_number, current_pastel_block_hash = check_current_pastel_blockchain_stats_func()
        artist_pastel_id_public_key = example_pastel_public_id_public_key_pem_format
        artist_receiving_blockchain_address = generate_new_pastel_blockchain_address_func()
        ...
        artwork_concatenated_image_hashes = artwork_concatenated_image_hashes.decode('utf-8')
        current_pastel_mining_difficulty_rate = get_current_pastel_mining_difficulty_func()
        average_pastel_mining_difficulty_rate_first_20k_blocks = get_avg_pastel_mining_difficulty_rate_first_20k_blocks_func()
        required_registration_fee_in_pastel_terms_post_difficulty_adjustment, required_registration_fee_in_pastel_terms_pre_difficulty_adjustment, pastel_mining_difficulty_adjustment_ratio, artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration = calculate_artwork_registration_fee_func(combined_image_size_in_mb)
        combined_artwork_image_and_metadata_string, combined_artwork_image_and_metadata_hash = generate_combined_artwork_image_and_metadata_string_and_hash_func(pastel_artwork_metadata_ticket_format_version_number, date_time_artwork_prepared_by_artist, current_bitcoin_block_number, current_bitcoin_block_hash, current_pastel_block_number, current_pastel_block_hash, artist_pastel_id_public_key, total_number_of_unique_copies_of_artwork_to_create, artwork_title, artist_name, artwork_series_name, artist_website, artist_statement, combined_image_size_in_mb, artwork_concatenated_image_hashes, current_pastel_mining_difficulty_rate, average_pastel_mining_difficulty_rate_first_20k_blocks, pastel_mining_difficulty_adjustment_ratio, required_registration_fee_in_pastel_terms_pre_difficulty_adjustment, required_registration_fee_in_pastel_terms_post_difficulty_adjustment, artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration)
        artist_signature_on_combined_artwork_image_and_metadata_string = sign_data_with_private_key_func(combined_artwork_image_and_metadata_string, example_pastel_public_id_private_key)
        registering_masternode_pastel_collateral_blockchain_address, registering_masternode_pastel_collateral_public_key, registering_masternode_pastel_collateral_private_key = get_my_masternode_collateral_address_and_key_pair_func()
        registering_masternode_collateral_signature_on_combined_artwork_image_and_metadata_string = sign_data_with_private_key_func(combined_artwork_image_and_metadata_string, registering_masternode_pastel_collateral_private_key)
        _, registering_masternode_pastel_id_private_key, _ = get_local_masternode_pastel_id_keypair_func()
        registering_masternode_pastel_id_signature_on_combined_artwork_image_and_metadata_string = sign_data_with_private_key_func(combined_artwork_image_and_metadata_string, registering_masternode_pastel_id_private_key)
        registering_masternode_pastel_blockchain_address_for_receiving_registration_fee = generate_new_pastel_blockchain_address_func()
        date_time_registering_masternode_processed_artwork = get_current_datetime_string_func()
        path_to_folder_containing_files_for_compression_dictionary = path_where_pastel_html_ticket_templates_are_stored
        pastel_zstd_compression_dictionary = generate_zstd_dictionary_from_folder_path_and_file_matching_string_func(path_to_folder_containing_files_for_compression_dictionary, '*_template.html')
        pastel_zstd_compression_dictionary_raw_data = pastel_zstd_compression_dictionary.as_bytes()
        artwork_metadata_ticket_pastel_blockchain_storage_address = generate_new_pastel_blockchain_address_func()
        pastel_blockchain_file_storage_compression_dictionary_pastel_blockchain_storage_address = generate_new_pastel_blockchain_address_func()
        pastel_blockchain_file_storage_compression_dictionary_file_hash = get_sha256_hash_of_input_data_func(pastel_zstd_compression_dictionary_raw_data)
        _, _, registering_masternode_pastel_id_public_key_pem_format = get_local_masternode_pastel_id_keypair_func()
        registering_masternode_ip_address = get_my_local_ip_func()
        nsfw_detection_model_file_hash = get_nsfw_model_file_hash_func()
        self.pastel_ticket_data_object_type = 'artwork_metadata_ticket'
        self.pastel_artwork_metadata_ticket_format_version_number = pastel_artwork_metadata_ticket_format_version_number
        ...
        self.current_pastel_block_number = current_pastel_block_number
        self.current_pastel_block_hash = current_pastel_block_hash
        self.artist_pastel_id_public_key = artist_pastel_id_public_key
        ...
        self.current_pastel_mining_difficulty_rate = current_pastel_mining_difficulty_rate
        self.average_pastel_mining_difficulty_rate_first_20k_blocks = average_pastel_mining_difficulty_rate_first_20k_blocks
        self.pastel_mining_difficulty_adjustment_ratio = pastel_mining_difficulty_adjustment_ratio
        self.required_registration_fee_in_pastel_terms_pre_difficulty_adjustment = required_registration_fee_in_pastel_terms_pre_difficulty_adjustment
        self.required_registration_fee_in_pastel_terms_post_difficulty_adjustment = required_registration_fee_in_pastel_terms_post_difficulty_adjustment
        self.artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration = artists_forfeitable_deposit_in_pastel_terms_to_initiate_registration
        ...
        self.registering_masternode_pastel_collateral_blockchain_address = registering_masternode_pastel_collateral_blockchain_address
        self.registering_masternode_pastel_id_public_key = registering_masternode_pastel_id_public_key_pem_format
        ...
        self.registering_masternode_pastel_id_signature_on_combined_artwork_image_and_metadata_string = registering_masternode_pastel_id_signature_on_combined_artwork_image_and_metadata_string
        self.registering_masternode_pastel_blockchain_address_for_receiving_registration_fee = registering_masternode_pastel_blockchain_address_for_receiving_registration_fee
        self.date_time_registering_masternode_processed_artwork = date_time_registering_masternode_processed_artwork
        self.artwork_metadata_ticket_pastel_blockchain_storage_address = artwork_metadata_ticket_pastel_blockchain_storage_address
        self.pastel_blockchain_file_storage_compression_dictionary_pastel_blockchain_storage_address = pastel_blockchain_file_storage_compression_dictionary_pastel_blockchain_storage_address
        self.pastel_blockchain_file_storage_compression_dictionary_file_hash = pastel_blockchain_file_storage_compression_dictionary_file_hash
        self.selected_art_folder_path = selected_art_folder_path

def get_pastel_html_ticket_template_string_func(pastel_html_ticket_type_string):
    global path_where_pastel_html_ticket_templates_are_stored
    glob_pattern_matching_string = path_where_pastel_html_ticket_templates_are_stored+'*'+pastel_html_ticket_type_string+'*.html'
    glob_result = glob.glob(glob_pattern_matching_string)
    if len(glob_result) == 1:
        path_to_pastel_html_ticket_file = glob_result[0]
        with open(path_to_pastel_html_ticket_file,'r') as f:
            pastel_html_ticket_template_string = f.read()
        return pastel_html_ticket_template_string
    else:
        print('Error, can\'nt find specified template file!')
        
def load_pastel_html_ticket_file_func(path_to_pastel_html_ticket_file):
    with open(path_to_pastel_html_ticket_file,'r') as f:
       pastel_ticket_html_string = f.read()
    return pastel_ticket_html_string

def generate_new_pastel_html_ticket_data_object_func(pastel_html_ticket_type_string):
    if pastel_html_ticket_type_string == 'artwork_metadata_ticket':
        pastel_ticket_data_object = fake_artwork_metadata_class()
    elif pastel_html_ticket_type_string == 'trade_ticket':
        pastel_ticket_data_object = fake_artwork_metadata_class() #Todo: write similar classes for these tickets
    elif pastel_html_ticket_type_string == 'treasury_fund_payment_request_voting_ticket':
        pastel_ticket_data_object = fake_artwork_metadata_class() #Todo: write similar classes for these tickets        
    elif pastel_html_ticket_type_string == 'offending_content_takedown_request_voting_ticket':
        pastel_ticket_data_object = fake_artwork_metadata_class() #Todo: write similar classes for these tickets       
    return pastel_ticket_data_object
...
def fill_in_new_pastel_html_ticket_data_object_func(pastel_ticket_data_object):
    pastel_html_ticket_type_string = getattr(pastel_ticket_data_object, 'pastel_ticket_data_object_type')
    selected_art_folder_path = getattr(pastel_ticket_data_object, 'selected_art_folder_path')
    pastel_html_ticket_template_html_string = get_pastel_html_ticket_template_string_func(pastel_html_ticket_type_string)
    pastel_html_ticket_template_html_string_parsed = lxml.html.fromstring(pastel_html_ticket_template_html_string)
    parsed_ticket_data = pastel_html_ticket_template_html_string_parsed.xpath('//td')
    ...
    pastel_html_ticket_data_field_values = [getattr(pastel_ticket_data_object, ticket_field_name) for ticket_field_name in parsed_ticket_field_names_list_filtered_again_processed]
    if len(parsed_ticket_field_names_list_filtered_again) == len(pastel_html_ticket_data_field_values):
        filled_in_pastel_html_ticket_html_string = pastel_html_ticket_template_html_string
        for field_count, current_template_field_data_placeholder_string in enumerate(parsed_ticket_field_names_list_filtered_again):
            current_template_field_data_value = pastel_html_ticket_data_field_values[field_count]
            filled_in_pastel_html_ticket_html_string = filled_in_pastel_html_ticket_html_string.replace(current_template_field_data_placeholder_string, str(current_template_field_data_value))
    if len(validation_type_fields_original) == len(validation_type_fields_clean):
        for field_count, current_validation_type_field_clean in enumerate(validation_type_fields_clean):
            ...
            filled_in_pastel_html_ticket_html_string = filled_in_pastel_html_ticket_html_string.replace(current_validation_type_field_original, current_validation_type_field_clean)
        pastel_ticket_output_file_path = os.path.join(selected_art_folder_path, pastel_html_ticket_type_string + '__'+ get_current_datetime_string_filepath_safe_func() + '.html')
        try:
            existing_ticket_file_paths = glob.glob(selected_art_folder_path + pastel_html_ticket_type_string + '*.html')
            if len(existing_ticket_file_paths):
                ...
                for current_existing_ticket_file_path in existing_ticket_file_paths:
                    try:
                        ...
                        print('Error: '+ str(e))
            with open(pastel_ticket_output_file_path,'w') as f:
                f.write(filled_in_pastel_html_ticket_html_string)
                print('Successfully wrote filled-in html ticket file to ' + pastel_ticket_output_file_path)
        except Exception as e:
            print('Error: '+ str(e))
        return filled_in_pastel_html_ticket_html_string, selected_art_folder_path, pastel_ticket_output_file_path
    else:
        print('Error! The number of placeholder fields does NOT match the number of available field values!')

def add_image_files_to_metadata_html_table_string_func(pastel_metadata_html_string, pastel_ticket_data_object, pastel_ticket_output_file_path=0):
    global nsfw_score_threshold
    max_image_preview_pixel_dimension = 600
    image_file_html_template_string = '<tr> <th valign="top"; align="left";><img src="PASTEL_IMAGE_FILENAME" alt="PASTEL_IMAGE_HASH" style="width:PASTEL_IMAGE_PIXEL_WIDTHpx; height:PASTEL_IMAGE_PIXEL_HEIGHTpx;"></th> <td valign="top";>PASTEL_IMAGE_HASH</td> <td valign="top";>PASTEL_IMAGE_NSFW_SCORE</td>  <td valign="top"; style="font-size: 1.5px;"> PASTEL_IMAGE_DUPE_FINGERPRINT_MODEL_1 </td> <td valign="top"; style="font-size: 1.5px;"> PASTEL_IMAGE_DUPE_FINGERPRINT_MODEL_2</td> <td valign="top"; style="font-size: 1.5px;"> PASTEL_IMAGE_DUPE_FINGERPRINT_MODEL_3</td> </tr>'
    new_html_lines = []
    selected_art_folder = pastel_ticket_data_object.selected_art_folder_path
    ...
    for current_image_file_path in list_of_paths_to_art_image_files:
        ...
        x = x.replace('PASTEL_IMAGE_FILENAME', './'+current_image_file_name)
        x = x.replace('PASTEL_IMAGE_HASH', sha256_hash_of_current_art_image_file)
        x = x.replace('PASTEL_IMAGE_PIXEL_WIDTH', str(preview_image_pixel_width))
        x = x.replace('PASTEL_IMAGE_PIXEL_HEIGHT', str(preview_image_pixel_height))
        x = x.replace('PASTEL_IMAGE_NSFW_SCORE', str(current_image_nsfw_score))
        x = x.replace('PASTEL_IMAGE_DUPE_FINGERPRINT_MODEL_1', current_model_1_image_fingerprint_vector_string)
        x = x.replace('PASTEL_IMAGE_DUPE_FINGERPRINT_MODEL_2', current_model_2_image_fingerprint_vector_string)
        x = x.replace('PASTEL_IMAGE_DUPE_FINGERPRINT_MODEL_3', current_model_3_image_fingerprint_vector_string)
    ...
    updated_pastel_metadata_html_string = pastel_metadata_html_string.replace('IMAGE_FILEPATHS_AND_HASH_DATA_AND_MORE', final_replacement_string)
    if not pastel_ticket_output_file_path == 0:
        try:
            with open(pastel_ticket_output_file_path,'w') as f:
                f.write(updated_pastel_metadata_html_string)
                print('Successfully added image data to filled-in html ticket file: ' + pastel_ticket_output_file_path)
        except Exception as e:
            print('Error: '+ str(e))
    return updated_pastel_metadata_html_string, dupe_detection_model_1, dupe_detection_model_2, dupe_detection_model_3, nsfw_graph_def
...
if 0:
    artwork_metadata_object = generate_new_pastel_html_ticket_data_object_func('artwork_metadata_ticket')
    filled_in_pastel_html_ticket_html_string, selected_art_folder_path, pastel_ticket_output_file_path = fill_in_new_pastel_html_ticket_data_object_func(artwork_metadata_object)
    updated_pastel_metadata_html_string, dupe_detection_model_1, dupe_detection_model_2, dupe_detection_model_3, nsfw_graph_def = add_image_files_to_metadata_html_table_string_func(filled_in_pastel_html_ticket_html_string, artwork_metadata_object, pastel_ticket_output_file_path )


if 0: #For debugging metadata functions:
    selected_art_folder = 'C:\\pastel\\art_folders_to_encode\\Arturo_Lopez__Number_02\\'
    pastel_ticket_data_object = artwork_metadata_object
    pastel_metadata_html_string = filled_in_pastel_html_ticket_html_string
    selected_art_folder = pastel_ticket_data_object.selected_art_folder_path
    list_of_paths_to_art_image_files = get_all_valid_image_file_paths_in_folder_func(selected_art_folder)
...
if 0: #Don't use unless the Dupe Detection database has problems:
    ...
    pastel_image_database_folder = 'C:\\Users\\jeffr\\Cointel Dropbox\\Pastel_Code\\dupe_detection_image_files_for_db\\'
    dupe_detection_model_1, dupe_detection_model_2, dupe_detection_model_3 = add_all_images_in_folder_to_image_fingerprint_database_func(pastel_image_database_folder)
