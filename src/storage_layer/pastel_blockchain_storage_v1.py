...
from pastel_utility_functions_v1 import generate_zstd_dictionary_from_folder_path_and_file_matching_string_func, compress_data_with_pastel_zstd_func, \
        decompress_data_with_pastel_zstd_func, get_all_pastel_directories_func, get_all_pastel_parameters_func
# pip install jsonrpc
...
temp_files_folder_path, path_to_zstd_compression_dictionaries, prepared_final_art_zipfiles_folder_path, chunk_db_file_path, file_storage_log_file_path, nginx_allowed_ip_whitelist_file_path, path_to_pastel_trade_ticket_template, \
...
def store_data_in_pastel_blockchain_func(input_data, pastel_zstd_compression_dictionary, receiving_pastel_blockchain_address):
    global rpc_connection_string, path_where_pastel_html_ticket_templates_are_stored
    ...
    assert(pastel_ticket_html_string == pastel_zstd_uncompressed_data.decode('utf-8')) #Check that we can decompress successfully
...
def retrieve_data_from_pastel_blockchain_func(blockchain_transaction_id):
    global rpc_connection_string, path_where_pastel_html_ticket_templates_are_stored
    rpc_connection = AuthServiceProxy(rpc_connection_string)
...
if 0: #Demo:
    ...
    if use_generate_new_compression_dictionary:
        pastel_zstd_compression_dictionary = generate_zstd_dictionary_from_folder_path_and_file_matching_string_func(path_where_pastel_html_ticket_templates_are_stored, '*ticket*.html')
    ...
    input_data = pastel_ticket_html_string
    blockchain_transaction_id = store_data_in_pastel_blockchain_func(input_data, pastel_zstd_compression_dictionary, receiving_pastel_blockchain_address)
    
    reconstructed_pastel_zstd_uncompressed_data = retrieve_data_from_pastel_blockchain_func(blockchain_transaction_id)
    reconstructed_pastel_zstd_uncompressed_data_decoded = reconstructed_pastel_zstd_uncompressed_data.decode('utf-8')
    
    assert(reconstructed_pastel_zstd_uncompressed_data_decoded == input_data)
