...
void CTxIndexProcessor::ProcessInputs(const CTransaction& tx, const uint32_t nTxOrderNo)
{
    ...
    const uint32_t nHeight = m_pBlockIndex->nHeight;
    ...
    for (uint32_t nTxIn = 0; nTxIn < tx.vin.size(); ++nTxIn)
    {
        ...
        if (fAddressIndex && scriptType != ScriptType::UNKNOWN)
        {
            ...
            m_vAddressIndex.emplace_back(
                CAddressIndexKey(scriptType, addrHash, nHeight, nTxOrderNo, txid, nTxIn, true),
                prevout.nValue * -1);
        ...
        if (fSpentIndex)
        {
            ...
            m_vSpentIndex.emplace_back(
                ...
                CSpentIndexValue(txid, nTxIn, nHeight, prevout.nValue, scriptType, addrHash));
        }
    }
}
...
void CTxIndexProcessor::ProcessOutputs(const CTransaction& tx, const uint32_t nTxOrderNo)
{
    ...
    const uint160& destBurnAddress = m_chainparams.getPastelBurnAddressHash();
    const uint256 &txid = tx.GetHash();
    const uint32_t nHeight = m_pBlockIndex->nHeight;
    ...
    for (uint32_t nTxOut = 0; nTxOut < tx.vout.size(); ++nTxOut)
    {
        ...
        if (fBurnTxIndex && (addrHash == destBurnAddress))
        {
            for (uint32_t nTxIn = 0; nTxIn < tx.vin.size(); ++nTxIn)
            {
                ...
                const ScriptType scriptTypeIn = prevout.scriptPubKey.GetType();
                if (scriptTypeIn == ScriptType::UNKNOWN)
                    continue;

                const uint160 addrHash = prevout.scriptPubKey.AddressHash();
                m_vBurnTxIndex.emplace_back(
                    CBurnTxIndexKey(scriptTypeIn, addrHash, nHeight, txid, nTxIn),
                    CBurnTxIndexValue(txout.nValue * -1, m_hashBlock, m_nBlockTime));
            }
        }
        ...
        m_vAddressIndex.emplace_back(
            CAddressIndexKey(scriptType, addrHash, nHeight, nTxOrderNo, txid, nTxOut, false),
            txout.nValue);

        // record unspent output
        m_vAddressUnspentIndex.emplace_back(
            CAddressUnspentKey(scriptType, addrHash, txid, nTxOut),
            CAddressUnspentValue(txout.nValue, txout.scriptPubKey, nHeight));
    }
}
...
bool CTxIndexProcessor::WriteIndexes(CValidationState& state)
{
    ...
    if (fBurnTxIndex)
    {
        if (!gl_pBlockTreeDB->UpdateBurnTxIndex(m_vBurnTxIndex))
...
void CTxIndexProcessor::UndoOutputs(const CTransaction& tx, const uint32_t nTxOrderNo)
{
    ...
    const uint160& destBurnAddress = m_chainparams.getPastelBurnAddressHash();
    const uint256 &txid = tx.GetHash();
    const uint32_t nHeight = m_pBlockIndex->GetHeight();
...
{
    if (!fAddressIndex && !fSpentIndex && !fBurnTxIndex)
		return;
    ...
    if (fAddressIndex || (fBurnTxIndex && m_BurnedAmount))
    {
        ...
        if (fBurnTxIndex && m_BurnedAmount)
        {
            m_vBurnTxIndex.emplace_back(
                CBurnTxIndexKey(scriptType, addrHash, m_pBlockIndex->GetHeight(), txid, nTxIn),
                CBurnTxIndexValue(m_BurnedAmount.value(), m_pBlockIndex->GetBlockHash(), m_nBlockTime));
        }
...
bool CTxIndexProcessor::EraseIndices(CValidationState& state)
{
    ...
    if (fBurnTxIndex)
    {
        if (!gl_pBlockTreeDB->EraseBurnTxIndex(m_vBurnTxIndex))
            return AbortNode(state, "Failed to delete burn transaction index");
    }
    if (fSpentIndex)
    {
        if (!gl_pBlockTreeDB->UpdateSpentIndex(m_vSpentIndex))
