#!/bin/bash
## Usage:
##  ./pcutil/build-debian-package.sh
...
PACKAGE_VERSION=$($SRC_PATH/src/pasteld --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/pasteld $DEB_BIN
cp $SRC_PATH/src/pastel-cli $DEB_BIN
cp $SRC_PATH/pcutil/fetch-params.sh $DEB_BIN/pastel-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/pasteld.1 $DEB_MAN
cp $SRC_DOC/man/pastel-cli.1 $DEB_MAN
cp $SRC_DOC/man/pastel-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/pasteld.bash-completion $DEB_CMP/pasteld
cp $SRC_PATH/contrib/pastel-cli.bash-completion $DEB_CMP/pastel-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/pasteld.1
gzip --best -n $DEB_MAN/pastel-cli.1
gzip --best -n $DEB_MAN/pastel-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/pasteld $DEB_BIN/pastel-cli
