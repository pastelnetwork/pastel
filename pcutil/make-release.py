...
def main(args=sys.argv[1:]):
    """
    Perform the final Pastel release process up to the git tag.
    """
...
def update_debian_changelog(release):
    ...
    os.environ['DEBFULLNAME'] = 'Pastel comunity'
    sh_log(
        'debchange',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "pastel-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "pastel-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './pastel-make-release.log'
    ...
    logging.info('pastel make-release.py debug log: %r', logname)
