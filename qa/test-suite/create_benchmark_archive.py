...
PASTEL_CLI = './src/pastel-cli'
USAGE = """
Requirements:
- find
- xz
- %s (edit PASTEL_CLI in this script to alter the path)
- A running mainnet pasteld using the default datadir with -txindex=1
...
def create_benchmark_archive(blk_hash):
    ...
    with open('benchmark/block-%d.dat' % blk['height'], 'wb') as f:
        f.write(binascii.unhexlify(subprocess.check_output([PASTEL_CLI, 'getblock', blk_hash, 'false']).strip()))

    txs = [json.loads(subprocess.check_output([PASTEL_CLI, 'getrawtransaction', tx, '1'])
                     ) for tx in blk['tx']]
    ...
    for tx in bar(unique_inputs.keys()):
        ...
        coins.extend(encode_varint(json.loads(
            subprocess.check_output([PASTEL_CLI, 'getblockheader', rawtx['blockhash']])
            )['height']))
