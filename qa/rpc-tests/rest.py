...
class RESTTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Block header length of 177 is: 285???
        ...
        assert_greater_than(int(response.getheader('content-length')), 285) #177
        response_str = response.read()

        # compare with block header
        response_header = http_get_call(url.hostname, url.port, '/rest/headers/1/'+bb_hash+self.FORMAT_SEPARATOR+"bin", True)
        assert_equal(response_header.status, 200)
        assert_equal(int(response_header.getheader('content-length')), 285) #177
        response_header_str = response_header.read()
        assert_equal(response_str[0:285], response_header_str)
