...
class TicketContractTest(PastelTestFramework):
    ...
    def contract_register(self, contract_id: str, subtype: str, secondary_key: Optional[str], address: Optional[str]) -> RegisterResultInfo:
        """
        ...
        result = self.nodes[0].tickets("register", *args)
    ...
    def contract_register_tests(self):
        ...
        info = self.contract_register("contract-1", "contract-subtype", "contract-secondary-key", self.v_addr[0])
        ...
        assert_raises_rpc(rpc.RPC_MISC_ERROR, "ticket transaction in mempool with the same secondary key",
            self.nodes[2].tickets, "register", ticket_type_name, "contract-2", "contract-subtype", "contract-secondary-key", self.v_addr[0])
    ...
    def contract_rpc_tests(self):
        ...
        v2 = []
        for i in range(3):
            v1.append(self.contract_register(f'v1-{i + 1}', 'subtype1', None, None))
            v2.append(self.contract_register(f'v2-{i + 1}', 'subtype2', None, None))
        self.generate_and_sync_inc(1)
        ...
        v3 = []
        for i in range(3):
            v3.append(self.contract_register(f'v3-{i + 1}', 'subtype3', f'seckey-{i+1}', None))
        self.generate_and_sync_inc(1)
